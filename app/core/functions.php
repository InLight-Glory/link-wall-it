...
function create_wall($side_id, $name) {
    ...
    $new_wall = [
        ...
        'name' => htmlspecialchars($name, ENT_QUOTES, 'UTF-8'),
        'access_control' => [
            'type' => 'public', // 'public', 'password', 'codelist', 'stripe'
            'password' => ['hash' => null, 'salt' => null],
            'codelist' => [],
            'stripe' => ['price_in_cents' => null, 'currency' => 'usd']
        ]
    ];
...
function update_wall_access(string $id, string $type, $value = null): bool {
    ...
    foreach ($db['walls'] as &$wall) {
        if ($wall['id'] === $id) {
            ...
            } elseif ($type === 'stripe' && is_array($value) && !empty($value['price_in_cents'])) {
                $wall['access_control']['type'] = 'stripe';
                $wall['access_control']['stripe']['price_in_cents'] = (int)$value['price_in_cents'];
                $wall['access_control']['stripe']['currency'] = strtolower($value['currency'] ?? 'usd');
            }
 ...
 * @return bool True on success, false on failure.
 */
function save_settings(array $new_settings): bool {
    $db = get_db();
    // Merge to ensure no existing settings are lost if not provided in the new settings
