...
if (isset($_GET['stripe_session_id'])) {
    ...
    try {
        $checkout_session = \Stripe\Checkout\Session::retrieve($_GET['stripe_session_id']);
        if ($checkout_session->payment_status === 'paid') {
            // Payment was successful. Grant access.
            if (!isset($_SESSION['unlocked_walls']) || !in_array($wall_id, $_SESSION['unlocked_walls'])) {
                $_SESSION['unlocked_walls'][] = $wall_id;
            }
            // Redirect to clean URL
            header("Location: wall.php?id=" . $wall_id . "&payment=success");
            exit;
        }
    } catch (Exception $e) {
        ...
        <main>
            ...
            <?php else: ?>
                <div class="access-form">
                    ...
                    <?php elseif ($access_type === 'stripe'): ?>
                        <p>This content is available for a small fee.</p>
                        <button class="share-btn" style="background-color: #6675e0; color: white; padding: 10px 20px; font-size: 1em;" onclick="goToStripeCheckout('<?= htmlspecialchars($settings['stripe_publishable_key']) ?>', '<?= htmlspecialchars($wall_id) ?>')">
                            Pay to Access (<?= htmlspecialchars(number_format($wall['access_control']['stripe']['price_in_cents'] / 100, 2)) ?> <?= strtoupper(htmlspecialchars($wall['access_control']['stripe']['currency'])) ?>)
                        </button>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
